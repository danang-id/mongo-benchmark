...
const BIN_FILE = './bin/mongobenchmark'
...
`
    ...
    fs.writeFileSync(BIN_FILE.concat('.cmd'), 'node %~dp0\\mongobenchmark', { encoding: ENCODING })
    fs.writeFileSync(BIN_FILE.concat('.sh'), 'node $(dirname "$0")/mongobenchmark', { encoding: ENCODING })
